...
org_html2.findAll('tr', limit=2)
headers = [th.getText() for th in org_html.findAll('tr', limit=2)[0].findAll('th')]
headers = headers[1:]
headers2 = [th.getText() for th in org_html2.findAll('tr', limit=2)[0].findAll('th')]
headers2 = headers2[1:]
headers
```
...
rows2 = org_html2.findAll('tr')[1:]
player_stats2 = [[td.getText() for td in rows[i].findAll('td')]
            for i in range(len(rows))]
data2 = pd.DataFrame(player_stats, columns = headers)
```
...
```{python}
data=data.drop(columns=['Pos', 'Age', 'Tm', 'G', 'GS', '2P', '3P', 'TRB', 'FT','PF'], axis=1)
```
```{python}
#| echo: false
#| output: false
